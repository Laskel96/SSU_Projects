...
sval.sival_int = (int)offset; //set offset
if(sigqueue(pid, SIGUSR1, sval) != 0){ //call sigqueue
person->watchers[i] = 0;
}
    }
}
...
watch_person (void)
{
  ...
  action.sa_flags = SA_SIGINFO; // set flags
  action.sa_handler = signal_received; // set handler
  if(sigaction( SIGINT , &action, NULL)) // set SIGINT handler
	fprintf(stdout, "failed to register signal handler for SIGINT\n");
  if(sigaction(SIGTERM, &action, NULL)) // set SIGTERM handler
	fprintf(stdout, "failed to register signal handler for SIGTERM\n");
  if(sigaction(SIGUSR1, &action, NULL)) // set SIGUSR! handler
	fprintf(stdout, "failed to register signal handler for SIGUSR1\n");
