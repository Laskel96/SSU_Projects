...
read_config (const char *filename)
{
	...
	while (fgets (line, sizeof (line), fp))
	{
		...
		*p = '\0';
		...
		s = p + 1;
		p = strchr (s, ':');
		if(!p) // if nothing here
			goto invalid_line;
		*p = '\0';
		strstrip(s);

		if(!((atoi(s) >=0 && atoi(s) <= 10000))){//4 decimal number
			MSG ("invalid number '%s' in line %d, ignored\n", s, line_nr);
			continue;
		}
		else if(s[0] == '\0' ){ // put random number
			srand(time(NULL));
			task.order = rand()%10000;//make 4decimal random number

		}else{
			task.order = atoi(s);//store oreder
		}
...
void sort(){
	Task * A = NULL;
	Task * B = NULL;
	for( A = tasks; A != NULL;  A = A->next)
		for(B = tasks; B != NULL; B = B->next)
			if(A->order >= B->order)
				swap(A,B);
}
		...
		char **argv)
{
	...
	sigemptyset (&sa.sa_mask); //reset signal set
	sigaddset(&sa.sa_mask, SIGCHLD); //add SIGCHLD signal


	sfd = signalfd(-1, &sa.sa_mask, 0); //get file discriptor

	fdsi.ssi_status = 0; // reset status
	
	if(sfd <0)
		MSG("fail for signalfd\n");

	if(sigprocmask(SIG_BLOCK, &sa.sa_mask, NULL) == -1){ //set sigprocmask block option
		MSG("error");
	}
	...
	while (!terminated)
	{
		...
		for (task = tasks; task != NULL; task = task->next){
			...
			if(s != sizeof(struct signalfd_siginfo)) //if empty
				MSG("failed to read signal");
			if(fdsi.ssi_signo == SIGCHLD){ // if sinal is SIGCHLD
				wait_for_children(SIGCHLD);//call wait
			}
