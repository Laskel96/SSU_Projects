...
thread_func (void *arg)
{
  ...
  do
    {
	if(tsem_try_wait(chopstick[i]) == 0){//get left chopstick
		if(tsem_try_wait(chopstick[k]) == 0){//get right chopstick
			update_status(i,1);	
			update_status(i,0);
			tsem_signal(chopstick[i]);
			tsem_signal(chopstick[k]);
		}else{// if can't get right chopstick
			tsem_signal(chopstick[i]);//put down left chopstick
		}
	}
    }
  while (1);
