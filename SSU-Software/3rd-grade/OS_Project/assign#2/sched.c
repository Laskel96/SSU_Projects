	...
	for (cpu_time = 0; p_done < process_total; cpu_time++)
	{
		...
		switch (sched)
		{
			...
			case SCHED_PR:
				...
				{
					int i; // for loop 
					int most_priority = PRIORITY_MAX+1;//store best priority
					
					for(i = 0; i < queue_len; i++)//for every processes in queue
						if(queue[i]->priority < most_priority)//if better than best priority
						{
							process = queue[i]; // set for next execution
							most_priority = process->priority; // set best priority
						}
				}	
				break;
			default:
