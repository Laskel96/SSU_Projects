...
testSam$label = as.factor(testSam$label)
  ...
  for(i in 1:30){
    predict <- knn(trainSam[,-1],testSam[,-1],trainSam$label,k=i)
...
plot(fmeasure_acc/10, type = "l", ylab = "fmeasure",
     Xlab = "K", main="fmeasure With Varying K")
