// Hill_Climbing.cpp : 콘솔 응용 프로그램에 대한 진입점을 정의합니다.
//
...
int main()
{
	...
	int start = 15;
	int goal = 1;
	node open[ElementSize * 3];
	node close[ElementSize *3];
	node path[ElementSize];
	int left_indicator = ElementSize * 3 - 1;
	int right_indicator = ElementSize * 3 - 1;
	bool exist = false;
	int close_indicator = 0;
	bool end = false;
	node child[ElementSize];
	...
	{
		child_indicator = 0;
		for (int i = 0; i < ElementSize; i++) { // initialize child, path
			child[i].cost = 0;
			child[i].f = 0;
			child[i].num = -1;
			child[i].num_parent = -1;
			path[i].num = -1;
			path[i].num_parent = -1;
			path[i].cost = 0;
		}
		for (int i = ElementSize - 1; i > -1; i--)
			if (W[X.num][i] != 0) // if there is child putit in child array
			{
				...
				if (!exist && (X.num != goal)) 
				{
					child[child_indicator].num = i;
					child[child_indicator].num_parent = X.num;
					child[child_indicator].cost = X.cost + W[X.num][i];
					child[child_indicator].f = child[child_indicator].cost + H[i];
					child_indicator++;
				}
				exist = false;
			}
		quicksort(child, 0, child_indicator - 1); // sort child
		for (int i = child_indicator-1; i > -1; i--) // put child into open start for smallest f value 
		{
			left_indicator--;
			open[left_indicator] = child[i];
		}
