// Breath_first.cpp : 콘솔 응용 프로그램에 대한 진입점을 정의합니다.
//
...
int main()
{
	...
	int right_indicator = 0;//point end of open
	node close[ElementSize];//no need to take out list
	...
	while (!end)//loop until iterator until open is empty
	{
		...
		if (X.num == goal) {
			...
			cout << "cost : " << cost << endl << endl << endl;//print cost;
			cost = 0;
		}
